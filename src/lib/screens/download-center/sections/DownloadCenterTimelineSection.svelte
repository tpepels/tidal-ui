<script lang="ts">
	...
	import type {
		DownloadCenterActionKeys,
		DownloadCenterCollapsibleSection,
		DownloadCenterJob,
		DownloadCenterStats
	} from '$lib/screens/download-center/downloadCenterSectionTypes';
	...
	type Props = {
		...
		processingJobs: DownloadCenterJob[];
		queuedJobs: DownloadCenterJob[];
		completedJobs: DownloadCenterJob[];
		resumableJobs: DownloadCenterJob[];
		activeSectionOpen: boolean;
		queueSectionOpen: boolean;
		failedSectionOpen: boolean;
		toggleSection: (section: DownloadCenterCollapsibleSection) => void;
		isJobActionPending: (jobId: string) => boolean;
		handlePauseJob: (job: DownloadCenterJob) => void | Promise<void>;
		handleCancelJob: (job: DownloadCenterJob) => void | Promise<void>;
		handleResumePausedJob: (job: DownloadCenterJob) => void | Promise<void>;
		handleRetryJob: (job: DownloadCenterJob) => void | Promise<void>;
		handleCopyFailureReport: (job?: DownloadCenterJob) => void | Promise<void>;
		handleRemoveJob: (job: DownloadCenterJob) => void | Promise<void>;
		handleClearFailed: () => void | Promise<void>;
		actionKeys: DownloadCenterActionKeys;
	...
	const matchesTypeFilter = (job: DownloadCenterJob, filter: JobTypeFilter): boolean => {
	...
	let completedAlbums = $derived(completedJobs.filter((job) => job.job.type === 'album').length);
