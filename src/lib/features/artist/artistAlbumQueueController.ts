...
type DownloadQueueJobStatus = 'queued' | 'processing' | 'paused' | 'completed' | 'failed' | 'cancelled';

type DownloadQueueJobPayload = {
...
type ArtistAlbumQueueAction = 'cancel' | 'resume';
...
type SendQueueAction = (jobId: string, action: ArtistAlbumQueueAction) => Promise<QueueActionResponse>;

type ArtistAlbumQueueControllerOptions = {
	getState: (albumId: number) => ArtistAlbumDownloadState;
	patchState: (albumId: number, patch: Partial<ArtistAlbumDownloadState>) => void;
...
function actionFailureMessage(action: ArtistAlbumQueueAction): string {
...
export function createDefaultArtistAlbumDownloadState(total = 0): ArtistAlbumDownloadState {
	return {
		status: 'idle',
		downloading: false,
		completed: 0,
...
export function isArtistAlbumQueueDownloadCancellable(
	state: ArtistAlbumDownloadState | undefined
): boolean {
	if (!state) return false;
	return isAlbumDownloadQueueActive(state.status);
}

export function resolveArtistAlbumQueueProgress(
	state: Pick<ArtistAlbumDownloadState, 'total' | 'completed'>,
	job: {
		trackCount?: number;
		completedTracks?: number;
		progress?: number;
...
export function reconcileArtistAlbumQueueJobState(
	current: ArtistAlbumDownloadState,
	job: DownloadQueueJobPayload
): {
	patch: Partial<ArtistAlbumDownloadState>;
	terminal: boolean;
} {
	const progress = resolveArtistAlbumQueueProgress(current, job);
...
export function createArtistAlbumQueueController(options: ArtistAlbumQueueControllerOptions) {
	...
	async function pollAlbumQueueJob(albumId: number, jobId: string, pollToken: number): Promise<void> {
		...
		try {
			...
			const next = reconcileArtistAlbumQueueJobState(current, payload.job);
	...
	async function cancelQueueDownload(albumId: number): Promise<QueueActionResponse> {
		...
		if (!isArtistAlbumQueueDownloadCancellable(state) || !state.queueJobId) {
