...
export interface RGBColorWithVibrancy extends RGBColor {
	vibrancy: number;
}
...
export const lighten = (color: RGBColor, amount: number): RGBColor =>
	mix(color, { red: 255, green: 255, blue: 255 }, amount);
export const darken = (color: RGBColor, amount: number): RGBColor =>
	mix(color, { red: 0, green: 0, blue: 0 }, amount);
...
export const ensureTextContrast = (color: RGBColor): RGBColor => {
	...
	while (
		(contrastWhite < MIN_CONTRAST_WHITE || contrastGrey < MIN_CONTRAST_GREY) &&
...
const selectAccentColor = (dominant: RGBColor, candidates: RGBColor[]): RGBColor => {
	...
	const ranking = (vivid.length > 0 ? vivid : candidates)
		.slice()
		.sort((a, b) => distance(dominant, b) - distance(dominant, a));
...
const loadColorThief = async (): Promise<ColorThiefInstance | null> => {
	...
	if (!colorThiefPromise) {
		colorThiefPromise = (
			import('colorthief') as Promise<ColorThiefModule | { default: ColorThiefConstructor }>
		)
			.then((module) => {
				const ctor =
					(module as { default?: ColorThiefConstructor }).default ??
					(module as ColorThiefConstructor);
				return new ctor();
			})
			.catch((error) => {
 ...
 */
export async function extractPaletteFromImageWebGL(
	url: string
): Promise<{ palette: RGBColor[]; mostVibrant: RGBColor }> {
	if (!browser) {
	...
	const palette: RGBColorWithVibrancy[] = [];
 ...
 */
const getAverageColorWithVibrancy = (
	ctx: CanvasRenderingContext2D,
	x: number,
	y: number,
	w: number,
	h: number
): RGBColor & { vibrancy: number } => {
